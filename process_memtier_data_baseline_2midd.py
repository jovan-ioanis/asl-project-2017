...
plots_base_name = "plots/baseline_2midd/"
file_base_name = "data/baseline_2midd/"
csv_file_name = "source_baseline_2midd.csv"
...
def make_filename(instance, cpt, wt, rep, readonly, vm):
    if readonly:
        c = "_S0-G1_"
    else:
        c = "_S1-G0_"
            ...
            for rep in range(1, repetitions + 1):
                ...
                for vm in range(1, memtier_vms + 1):
                    ...
                    for inst in range(1, memtier_instances_per_vm + 1):
                        with open(make_filename(inst, cpt, wt, rep, readonly, vm)) as json_file:
    ...
    for j in range(len(a)):
        ...
        for cpt in virtual_clients_pt:
            ...
            for wt in worker_threads:
                ...
                restime_stdev = np.std(np.asarray(restimes))

                full_data[a[j]][cpt][wt] = [throughput_means, throughput_stdev, restime_means, restime_stdev]

    for j in range(len(d)):
        path = plots_base_name + "memtier_logs_baseline_2midd_" + d[j] + ".csv"
