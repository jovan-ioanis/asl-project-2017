...
copy_all_files_from_memtiers() {
	...
	connect_fast ${memtier[1]} "$command_for_memtier_kill"
	copyFilesBackFromMemtier $nethzid $nethzid$vm_name_base${memtier[0]}$suffix
	copyFilesBackFromMemtier $nethzid $nethzid$vm_name_base${memtier[1]}$suffix
	connect_fast ${memtier[0]} " rm -r json* dstat* client_stats* "
	connect_fast ${memtier[1]} " rm -r json* dstat* client_stats* "
...
ping_test() {
	...
	connect_fast ${memtier[1]} "ping -c 1 "${private_ips_middleware[0]}" " > $dst_folder_basename$subfolder"/ping_results_memtier2_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
