...
memtier_clients_pt=(1 5 8 15 22 28 32 42 52 64)			
...
subfolder="/experiment_write_only"
...
# param1: client per thread count, param2: worker thread count, param3: repetition
run_all_memtiers() {
...
# param1: client per thread count, param2: worker threads, param3: repetition, param4: 1 for "r" or 0 for "w"
run_remote_dstat() {
	...
	command_for_dstat=" dstat -c -d -i -l -m -n -p -t -y --tcp --output dstat_memtier2_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
	connect_fast ${memtier[1]} "$command_for_dstat" > /dev/null &

	echo "                >> Running dstat at "${memtier[2]}" .."
	command_for_dstat=" dstat -c -d -i -l -m -n -p -t -y --tcp --output dstat_memtier3_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
	...
	command_for_dstat=" dstat -c -d -i -l -m -n -p -t -y --tcp --output dstat_memcached2_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
	connect_fast ${memcached[1]} "$command_for_dstat" > /dev/null &

	echo "                >> Running dstat at "${memcached[2]}" .."
	command_for_dstat=" dstat -c -d -i -l -m -n -p -t -y --tcp --output dstat_memcached3_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
...
ping_test() {
	...
	connect_fast ${memtier[1]} "ping -c 1 "${private_ips_middleware[1]}" " > $dst_folder_basename$subfolder"/ping_results_memtier3_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
	...
	connect_fast ${middleware[1]} "ping -c 1 "${private_ips_memcached[1]}" " > $dst_folder_basename$subfolder"/ping_results_middleware2_cpt"$1"_wt"$2"_rep"$3"_"$4".txt"
	echo "                >> Ping test done."
}
